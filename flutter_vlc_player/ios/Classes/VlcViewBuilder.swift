...
public class VLCViewBuilder: NSObject, VlcPlayerApi{
    ...
    init(registrar: FlutterPluginRegistrar) {
        ...
        NSLog("[flutter_vlc_player][iOS] VlcPlayerApiSetup registered in VLCViewBuilder.init")
    }
    
    public func build(frame: CGRect, viewId: Int64) -> VLCViewController{
        // Re-register handlers defensively for engines where channel setup may be
        // lost or delayed before the first platform view is created.
        VlcPlayerApiSetup(messenger, self)
        NSLog("[flutter_vlc_player][iOS] VlcPlayerApiSetup ensured in build(viewId=\(viewId))")
        //
        ...
        NSLog("[flutter_vlc_player][iOS] Stored VLC player viewId=\(viewId) (players=\(players.count))")
        return vlcViewController;
    }
    
    public func initialize(_ error: AutoreleasingUnsafeMutablePointer<FlutterError?>) {
        NSLog("[flutter_vlc_player][iOS] VlcPlayerApi.initialize")
        return
    }
    ...
    public func create(_ input: CreateMessage, error: AutoreleasingUnsafeMutablePointer<FlutterError?>) {
        NSLog("[flutter_vlc_player][iOS] VlcPlayerApi.create viewId=\(String(describing: input.viewId)) uri=\(input.uri ?? "")")
        
        let player = getPlayer(viewId: input.viewId)
        if player == nil {
            NSLog("[flutter_vlc_player][iOS] create called before player view exists for viewId=\(String(describing: input.viewId))")
        }
    ...
    public func dispose(_ input: ViewMessage, error: AutoreleasingUnsafeMutablePointer<FlutterError?>) {
        NSLog("[flutter_vlc_player][iOS] VlcPlayerApi.dispose viewId=\(String(describing: input.viewId))")
        
        let player = getPlayer(viewId: input.viewId)
        
        player?.dispose()
        players.removeValue(forKey: input.viewId as! Int)
        NSLog("[flutter_vlc_player][iOS] dispose complete viewId=\(String(describing: input.viewId)) (players=\(players.count))")
    }
